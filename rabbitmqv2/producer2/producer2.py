...
routing_key = 'fan'  # This can be any key that matches the routing.
channel.queue_bind(exchange=exchange_name, queue=queue_name, routing_key=routing_key)

message_number = 1
while True:
    message = f'Hello, RabbitMQ! Message #{message_number} FANOUT'
    
    # Send a message to the exchange with the specified routing key.
    channel.basic_publish(exchange=exchange_name, routing_key='fan2', body=message)
